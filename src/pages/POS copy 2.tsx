...
export default function POS() {
    ...
    return (
        <div className={`${darkMode ? 'dark' : ''} flex flex-col md:flex-row h-screen bg-gray-100 dark:bg-gray-900 font-sans relative`}>
            ...
            {/* PANIER */}
            <div className={`w-full md:w-80 bg-white/80 dark:bg-gray-900/80 backdrop-blur-md p-4 flex flex-col shadow-lg transition-transform duration-300 ${isMobile && !mobileCartOpen ? '-translate-x-full absolute top-0 right-0 h-full z-50' : 'translate-x-0'}`}>
                <div className="cart-target mb-3 font-bold text-lg text-gray-700 dark:text-gray-100 flex justify-between items-center">
                    Panier ({cart.length})
                    {isMobile && <button onClick={() => setMobileCartOpen(false)}><X className="text-gray-800 dark:text-gray-200"/></button>}
                </div>
